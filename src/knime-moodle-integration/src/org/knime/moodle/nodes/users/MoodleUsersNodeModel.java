 ...
 * @author Fran Gil
 */
public class MoodleUsersNodeModel extends NodeModel {
	 ...
	 */
	private static final NodeLogger LOGGER = NodeLogger.getLogger(MoodleUsersNodeModel.class);
	 ...
	 */
	protected MoodleUsersNodeModel() {
	...
	protected PortObject[] execute(final PortObject[] inObjects, final ExecutionContext exec)
		...
		cols[3] = new DataColumnSpecCreator("firstaccess", IntCell.TYPE).createSpec();
		cols[4] = new DataColumnSpecCreator("lastcourseaccess", IntCell.TYPE).createSpec();
		cols[5] = new DataColumnSpecCreator("lastaccess", IntCell.TYPE).createSpec();
		cols[6] = new DataColumnSpecCreator("roles", StringCell.TYPE).createSpec();
		cols[7] = new DataColumnSpecCreator("country", StringCell.TYPE).createSpec();
		cols[8] = new DataColumnSpecCreator("city", StringCell.TYPE).createSpec();
		...
		for(int j = 0; j < coursesID.size(); ++j ) {		
		    ...
		    // Obtener usuarios
			JSONArray users = UtilMethods.getJSONArrayResponse(webService, new CoreEnrolGetEnrolledUsers(courseid));
		    ...
		    for (int i = 0; i < users.length(); ++i) {
				...
				for (int k = 0; k < rolesArray.length(); k++) {
					JSONObject rolesObject = rolesArray.getJSONObject(k);
					rolesObject.get("shortname");
